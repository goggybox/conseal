...
const DesktopResolutions = ['1366x768', '1440x900', '1600x900', '1920x1080', '1920x1200', '2560x1440', '2560x1600', '3840x2160'];
const MacResolutions = ['1920x1080', '2560x1600', '4096x2304', '5120x2880'];

// operating systems to randomize hardware for
const randomHW = ['win1', 'win2', 'win3', 'win4', 'lin1', 'lin2', 'lin3'];

let getBadge = (browser) => {
...
export class Generator {
  browsers = {
    ...
    edg: (os) => {
      ...
      let resolutions = os.id.includes('mac') ? MacResolutions : DesktopResolutions;
      let screenRes = resolutions[Math.floor(Math.random() * resolutions.length)].split('x').map(Number);

      let ua = `Mozilla/5.0 (${platform}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${chromeVersion} Safari/537.36 Edg/${version}`;

      let hardwareConcurrency = randomHW.includes(os.id) ? (Math.random() > 0.5 ? 4 : 2) : 4;

      return {
        accept: {
          header: 'text/html,application/xhtml+xml,application/xml; =0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
    ...
    // edge (mobile)
    edgm: (os) => {
      let versions = BrowserVersions.edg;

      const device = devices.getDevice('mobile', os.id);

      let screenRes = device.viewport.split('x').map(Number);
      let ua = `Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${versions.androidChrome} Mobile Safari/537.36 EdgA/${versions.android}`;
    ...
    // firefox esr
    esr: (os) => {
      let version = BrowserVersions.esr.desktop;
      let appVersion;
      let platform;
    ...
    esr2: (os) => {
      let version = BrowserVersions.esr2.desktop;
      let appVersion;
      let platform;
    ...
    // firefox
    ff: (os) => {
      let version = BrowserVersions.ff.desktop;
      let appVersion;
      let platform;
    ...
    // firefox for android
    ffm: (os) => {
      let version = BrowserVersions.ff.mobile;
      let ua;
    ...
    gcr: (os) => {
      ...
      let ua = `Mozilla/5.0 (${platform}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${version} Safari/537.36`;
    ...
    gcrm: (os) => {
      let versions = BrowserVersions.gcr;
      let ua;
    ...
    // safari
    sf: (os) => {
      let version = BrowserVersions.sf[os.id];
      ...
      let hardwareConcurrency = Math.random() > 0.5 ? 4 : 8;
